export function parseReaperProject(fileContent) {
  const result = {
    offset: 0,
    mainMarkers: [],
    tracks: [],
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inSourceBlock && trimmedLine.startsWith('FILE ') && currentTrack && inItem) {
        ...
        if (fileMatch) {
            ...
            if (lowerF.match(/\.(wav|mp3|m4a|flac|aiff|ogg|mp4|mov|avi|mkv)$/)) {
                if (!isTrackMuted && !isItemMuted) {
                    const tName = currentTrack.name.toLowerCase();
                    if (!tName.includes('mtc') && !tName.includes('ltc') && !tName.includes('timecode')) {
